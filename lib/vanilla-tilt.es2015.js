...
class VanillaTilt {
  ...
  update() {
    ...
    const defaultX = this.settings.resetToStarting ? this.settings.startX : 0;
    const defaultY = this.settings.resetToStarting ? this.settings.startY : 0;

    this.element.style.transform = "perspective(" + this.settings.perspective + "px) " +
      "rotateX(" + (this.settings.axis === "x" ? defaultX : values.tiltY) + "deg) " +
      "rotateY(" + (this.settings.axis === "y" ? defaultY : values.tiltX) + "deg) " +
